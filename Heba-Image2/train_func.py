...
def parse_arguments():
    ...
    parser.add_argument('--save_dir',action='store', default='checkpoint_vgg19.pth') # THIS IS PROBLEMATIC AS YOU STILL NAME YOUR MODEL AFTER VGG19 EVEN IF YOU HAVE BUILT A DENSENET MODEL E.G. 
...
def save_checkpoint(Test_accuracy, model, architect, train_data, output_layer, hidden_layer, optimizer, epochs, learningrate, Dropout, checkpoin_path):
    #if Test_accuracy >= 0.70: # THIS LINE IS GOOD TO BE COMMENTED OUT FOR REVIEW
        model.class_to_idx = train_data.class_to_idx
        torch.save(model.state_dict(), checkpoin_path)
        checkpoint= {'input_size':arch[architect],
                     'output_size':output_layer,
                     'hidden_layers':hidden_layer, # YOU HAD THIS VARIABLE DIFFERENT IN YOUR LOAD FUNCTION LOAD FUNCTION AND SAVE FUNCTIONS VARIABLES MUST ALIGN
